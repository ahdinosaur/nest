...
fn set_simple() {
    ...
    assert_eq!(
        store
            .set(&["hello", "world", "nest"], &json!(1).into())
            .unwrap(),
        (),
    );
    assert_eq!(
    ...
    assert_eq!(
        store
            .set(&["hello", "world"], &json!({ "nest": 2 }).into())
            .unwrap(),
        (),
    );
    assert_eq!(
    ...
    assert_eq!(
        store
            .set(&["hello"], &json!({ "world": { "nest": 3 } }).into())
            .unwrap(),
        (),
    );
    assert_eq!(
...
"#;
    ...
    assert_eq!(
        store
            .set(&["hello", "world", "nest"], &json!(true).into())
            .unwrap(),
        (),
    );
    temp_0.child("hello/world.json").assert(expected);
    temp_0.close().unwrap();
    ...
    assert_eq!(
        store
            .set(&["hello", "world"], &json!({ "nest": true }).into())
            .unwrap(),
        (),
    );
    temp_1.child("hello/world.json").assert(expected);
    temp_1.close().unwrap();
    ...
    assert_eq!(
        store
            .set(&["hello"], &json!({ "world": { "nest": true } }).into())
            .unwrap(),
        (),
    );
    temp_2.child("hello/world.json").assert(expected);
    temp_2.close().unwrap();
    ...
    temp_3.child("hello/world.json").assert(expected);
    temp_3.close().unwrap();
}
